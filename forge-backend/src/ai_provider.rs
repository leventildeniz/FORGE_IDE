...
pub async fn handle_ai_chat_stream(
    ...
    // 1b. Initialize MCP Servers and Fetch Tools
    let mut mcp_clients: std::collections::HashMap<
        String,
        std::sync::Arc<crate::mcp_client::McpClient>,
    > = std::collections::HashMap::new();
    let mut mcp_tools_text = String::new();

    if let Some(servers) = mcp_servers {
        for srv in servers {
            if let (Some(name), Some(command), Some(args_val), Some(is_enabled)) = (
                srv.get("name").and_then(|v| v.as_str()),
                srv.get("command").and_then(|v| v.as_str()),
                srv.get("args").and_then(|v| v.as_array()),
                srv.get("isEnabled").and_then(|v| v.as_bool()),
            ) {
                if is_enabled {
                    ...
                    if let Ok(_) = client.start(command, &args).await {
                        // Give the MCP server time to initialize (npx downloads can take a while on first run)
                        let mut retries = 10;
                        let mut found_tools = false;

                        while retries > 0 && !found_tools {
                            tokio::time::sleep(tokio::time::Duration::from_secs(2)).await;
                            // Ask server for its tools
                            match client.send_request("tools/list", None).await {
                                Ok(res) => {
                                    if let Some(tools) = res.get("tools").and_then(|v| v.as_array())
                                    {
                                        found_tools = true;
                                        for tool in tools {
                                            if let (Some(t_name), Some(t_desc)) = (
                                                tool.get("name").and_then(|v| v.as_str()),
                                                tool.get("description").and_then(|v| v.as_str()),
                                            ) {
                                                mcp_tools_text.push_str(&format!(
                                                    "   - Server `{}` -> Tool `{}`: {}\n     Usage: @@MCP: {} | {} | {{...json_arguments...}}@@\n",
                                                    name, t_name, t_desc, name, t_name
                                                ));
                                            }
                                        }
                                        crate::debug_log!(
...
3. WEB AGENT (Internet Access & Scraper):
   - To search the web: @@WEB: search <query>@@ (e.g. @@WEB: search React 19 features@@)
   - To read a website (extracts clean markdown, drops HTML): @@WEB: fetch <url>@@ (e.g. @@WEB: fetch https://google.com@@)
...
- If the user asks a question, gather context autonomously before answering."#;
    ...
    // Append Dynamic MCP Tools if any
    if !mcp_tools_text.is_empty() {
        system_text.push_str("\n\n4. MCP AGENT (External Tools & Internet Access):\n");
        system_text.push_str("You have access to the following external tools via the Model Context Protocol (MCP).\n");
        system_text.push_str("To execute them, use the exact syntax shown below. The system will run it and return the result.\n\n");
        system_text.push_str(&mcp_tools_text);
    }
    ...
    let mut proj_root_path = String::new();
    if let Some(ctx_obj) = context.as_object() {
        if let Some(pr) = ctx_obj.get("projectRoot").and_then(|v| v.as_str()) {
            proj_root_path = pr.to_string();
        }
    }

    if !proj_root_path.is_empty() {
        let memory_dir = std::path::Path::new(&proj_root_path)
            .join(".forge")
            .join("knowledge");

        // Auto-create directory and memory files if they don't exist
        if !memory_dir.exists() {
            let _ = std::fs::create_dir_all(&memory_dir);
        }

        let context_path = memory_dir.join("context.md");
        ...
        let memory_files = ["context.md", "decisions.md"];
        ...
        for file_name in memory_files {
            ...
            if file_path.exists() {
                if let Ok(content) = std::fs::read_to_string(&file_path) {
                    ...
                    // We allocate 1024 tokens explicitly for each memory file to ensure it's usually visible.
                    let char_limit = 1024 * 4;
                    ...
                    memory_text.push_str(&header);
                    memory_text.push_str(&truncated_content);
                    memory_text.push_str(footer);
                }
            }
        }
    }

    context_text.push_str(&memory_text);
            ...
            Ok(res) => {
                ...
                if let Some((cmd, arg)) = code_command {
                    ...
                    if cmd == "memory" {
                        ...
                        if parts.len() == 3 {
                            let srv_name = parts[0].trim();
                            let tool_name = parts[1].trim();
                            let json_args_str = parts[2].trim();

                            if let Some(client) = mcp_clients.get(srv_name) {
                                let parsed_args: Value = serde_json::from_str(json_args_str)
                                ...
                                match tokio::time::timeout(
                                    ...
                                    client.send_request("tools/call", Some(params)),
                                )
                                .await
                                {
                                    Ok(Ok(res)) => {
                                        // Result is typically { "content": [ { "type": "text", "text": "..." } ] }
                                        let content_str = if let Some(content_arr) =
                                            res.get("content").and_then(|v| v.as_array())
                                        {
                                        ...
                                        result_text = format!("MCP Tool Result:\n{}", content_str);
                                        ui_feedback = format!(
                                            "> 🔌 **MCP Tool Called:** `{}/{}`\n\n",
                                            srv_name, tool_name
                                        );
                                    ...
                                    Ok(Err(e)) => {
                                        let err_obj = serde_json::json!({
                                            ...
                                            "success": false,
                                            "error": e.to_string(),
                                            "suggestion": "Check MCP server logs or parameters provided to the tool."
                                        });
                                        result_text = serde_json::to_string_pretty(&err_obj)
                                            .unwrap_or_else(|_| e.to_string());
                                        ui_feedback = format!(
                                            "> ❌ **MCP Tool Error:** `{}/{}` ({})\n\n",
                                            srv_name, tool_name, e
                                        );
                                    }
                                    Err(_) => {
                                        let err_obj = serde_json::json!({
                                            ...
                                            "suggestion": "The MCP tool took too long to respond."
                                        });
                                        result_text = serde_json::to_string_pretty(&err_obj)
                                            .unwrap_or_else(|_| "Timeout".to_string());
                                        ui_feedback = format!(
                                            "> ⏱️ **MCP Tool Timed Out:** `{}/{}`\n\n",
                                            srv_name, tool_name
                                        );
                                    }
                                }
                                ...
                                let err_obj = serde_json::json!({
                                    ...
                                    "suggestion": "Verify that the MCP server is configured and successfully started."
                                });
                                result_text = serde_json::to_string_pretty(&err_obj)
                            ...
                            let err_obj = serde_json::json!({
                                "success": false,
                                "error": "Invalid MCP syntax.",
                                "suggestion": "Expected `@@MCP: server | tool | {...}@@` syntax."
                            });
                            result_text = serde_json::to_string_pretty(&err_obj)
                                .unwrap_or_else(|_| "Invalid Syntax".to_string());
                            ui_feedback = format!("> ❌ **Invalid MCP Syntax**\n\n");
                        }
                    } else if cmd == "search" {
                        ...
                        if parts.is_empty() {
                            ...
                            if action == "search" {
                                ...
                                match crate::knowledge::search_web(target).await {
                                    ...
                                    Err(e) => {
                                        result_text = format!("Web search failed: {}", e);
                                    }
                                }
                                ...
                                match crate::knowledge::scrape_url(target).await {
                                    Ok(res) => {
                                        // Truncate massively long pages
                                        let char_limit = 40000;
                                        if res.len() > char_limit {
                                            result_text = format!(
                                                "Website Content ({}):\n{}... [TRUNCATED]",
                                                target,
                                                &res[..char_limit]
                                    ...
                                    Err(e) => {
                                        result_text = format!("Web fetch failed: {}", e);
                                    }
                                }
                            } else {
    ...
    // Stop MCP Clients when chat stream finishes
...
pub async fn generate_commit_message(
    ...
    let mut system_prompt = "You are an expert AI programming assistant generating Git commit messages. Follow good Git style: short subject line, blank line, then concise body if needed. ONLY return the commit message, no markdown formatting blocks.".to_string();

    let agents_md = std::path::Path::new(project_root).join("AGENTS.md");
    ...
    let messages = vec![
        ...
        json!({
            "role": "user",
            "content": format!("Generate a git commit message for the following changes:\n\n```diff\n{}\n```\n\nOnly output the commit message.", diff)
        }),
    ];
    ...
    let mut req = client.post(format!("{}/chat/completions", endpoint));

    if let Some(key) = model.get("apiKey").and_then(|v| v.as_str()) {
        if !key.is_empty() {
    ...
    if !res.status().is_success() {
        ...
        return Err(format!("Model API error: {}", err));
    }

    let resp_json: Value = res.json().await.map_err(|e| e.to_string())?;

    if let Some(choices) = resp_json.get("choices").and_then(|v| v.as_array()) {
        if let Some(first) = choices.get(0) {
            if let Some(msg) = first.get("message").and_then(|v| v.as_object()) {
                if let Some(content) = msg.get("content").and_then(|v| v.as_str()) {
                    let mut final_msg = content.trim().to_string();
                    // Clean up markdown quotes if model added them
                    if final_msg.starts_with("```") {
                        let lines: Vec<&str> = final_msg.lines().collect();
                        if lines.len() >= 2 {
                            let end = if final_msg.ends_with("```") {
                                lines.len() - 1
                            } else {
                                lines.len()
                            };
                            final_msg = lines[1..end].join("\n").trim().to_string();
                        }
                    }
                    return Ok(final_msg);
                }
